...
with open('process_list_robin.txt') as f:
    lines = f.readlines()
...
#     plt.title('Freestream');plt.xlim(0,3);plt.ylim(-1,2)
    ...
    umaxinst=np.zeros((SR*time,8))
    umaxinst[:,0]=Time
    umaxinst[:,1]=Uflc_filt[:,0]
    umaxinst[:,2]=Uflc_filt[:,5]
    umaxinst[:,3]=Uflc_filt[:,10]
    umaxinst[:,4]=Uflc_filt[:,15]
    umaxinst[:,5]=Uflc_filt[:,20]
    umaxinst[:,6]=Uflc_filt[:,loc]
    umaxinst[:,7]=Uflc_filt[:,-1]

    
    
    a=str(Uinf)
    np.savetxt('uflc_' + a[0:6] + '_'+str(con[:-1])+'_'+str(xloc)+'.txt',umaxinst,delimiter =' ')
    profile=np.zeros((30,8))
    profile[:,0]=y_corr/disp_t
    profile[:,1]=U_norm
    profile[:,2]=Urms/Uinf
    profile[:,3]=Urms_filt/Uinf
    profile[:,4]=Urmsp/Uinf
    profile[:,5]=Urmsn/Uinf
    profile[:,6]=y_corr
    profile[:,7]=Vel
