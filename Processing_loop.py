...
with open('process_list_test.txt') as f:
    lines = f.readlines()
...
#     plt.title('Freestream');plt.xlim(0,3);plt.ylim(-1,2)
    ...
    print(Uinf)
    a=str(Uinf)
    np.savetxt('uflc_' + a[0:6] + '_893_wall.txt',umaxinst,delimiter =' ')
    ...
    np.savetxt('profile_' + a[0:5] + '_893_max.txt',profile,delimiter =' ')
    print(tp.process_time() - start)